 ...
 *
 */
public class TripleBT  implements GlobalConst{
           ...
           ConvertException
    {
      ...
      try{
        ...
        if( entry.data instanceof IndexData )
          n+=4;
        else if (entry.data instanceof TripleLeafData )      
          n+=8;
        ...
        if ( entry.data instanceof IndexData ) {
          ...
          Convert.setIntValue( ((TripleLeafData)entry.data).getData().slotNo,
                               m, data);
          Convert.setIntValue( ((TripleLeafData)entry.data).getData().pageNo.pid,
                               m+4, data);
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    {
      TripleBTSortedPage sortedPage=new TripleBTSortedPage(pageno, keyType);
      int i;
      i=0;
      if ( sortedPage.getType()==NodeType.INDEX ) {
        TripleBTIndexPage indexPage=new TripleBTIndexPage((Page)sortedPage, keyType);
        System.out.println("");
        ...
        TripleBTLeafPage leafPage=new TripleBTLeafPage((Page)sortedPage, keyType);
        System.out.println("");
        ...
        for(KeyDataEntry entry=leafPage.getFirst(tid); entry!=null; 
          ...
          if( keyType==AttrType.attrInteger) 
            System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
                               (IntegerKey)entry.key+ ",  "+(TripleLeafData)entry.data+ " )");
          if( keyType==AttrType.attrString) 
            System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
                               (StringKey)entry.key + ",  "+(TripleLeafData)entry.data); 
  ...
  private static void _printTree(PageId currentPageId, String prefix, int i, 
           ...
           ReplacerException 
    {
      ...
      TripleBTSortedPage sortedPage=new TripleBTSortedPage(currentPageId, keyType);
      prefix=prefix+"       ";
      i++;
      if( sortedPage.getType()==NodeType.INDEX) {  
        TripleBTIndexPage indexPage=new TripleBTIndexPage((Page)sortedPage, keyType);
   ...
   *@exception ReplacerException  error from lower layer
   */
  public static void printAllLeafPages(TripleBTreeHeaderPage header) 
    throws IOException, 
           ConstructPageException, 
           IteratorException,
           HashEntryNotFoundException,
