...
/**
 * A TripleBTLeafPage is a leaf page on a label B+ tree.  It holds abstract 
 * <key, TID> pairs; it doesn't know anything about the keys 
 * (their lengths or their types), instead relying on the abstract
 * interface consisting of TripleBT.java.
 */
public class TripleBTLeafPage extends TripleBTSortedPage {
   ...
   */
  public TripleBTLeafPage(PageId pageno, int keyType) 
    throws IOException, 
  ...
  /**associate the TripleBTLeafPage instance with the Page instance,
   * also it sets the type to be NodeType.LEAF. 
   ...
   *@exception ConstructPageException BTLeafPage constructor error
   */
  public TripleBTLeafPage(Page page, int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
   ...
   *@exception ConstructPageException BTLeafPage constructor error
   */
  public TripleBTLeafPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
   ...
   *@param tid It will be modified and the first rid in the leaf page
  ...
  public KeyDataEntry getFirst(TID tid) 
    throws  IteratorException
    {
      ...
      try {
        ...
        entry= TripleBT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0),
    ...
    *@param tid It will be modified and the next rid will be passed out 
     ...
     throws  IteratorException
   {
	 ...
	 return null;
       ...
       entry= TripleBT.getEntryFromBytes(getpage(),getSlotOffset(i), getSlotLength(i),
                  keyType, NodeType.LEAF);
   ...
   */
  boolean redistribute(TripleBTLeafPage leafPage, TripleBTIndexPage parentIndexPage, 
	...
	if (direction ==-1) { // 'this' is the left sibling of leafPage
	  ...
	  if ( (getSlotLength(0) + available_space()+ 8) > ((MAX_SPACE-DPFIXED)/2)) {
            ...
            KeyDataEntry firstEntry;
            firstEntry= TripleBT.getEntryFromBytes(getpage(), getSlotOffset(0),
